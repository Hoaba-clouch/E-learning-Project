...
### 4.4 Khởi tạo database mới

Các script trong `database` gồm schema gốc, migration và dữ liệu mẫu. Thứ tự đề
xuất cho môi trường kiểm thử mới:
...
14. Tùy chọn: `database/seed_instructor_teacher4.sql`
...
> Cảnh báo: `database/mock_data.sql` có nhiều lệnh `TRUNCATE`. Không chạy trên
> database có dữ liệu cần giữ.

Nếu database đã được bàn giao sẵn, không chạy lại `mock_data.sql`; chỉ chạy
migration còn thiếu sau khi sao lưu.
